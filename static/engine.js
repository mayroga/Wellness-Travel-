...
// =========================================================================
    ...
    // POOL ESTRICTO DE LAS 48 PREGUNTAS ÉLITE DEL ORÁCULO
    POOL_COMPLETO_48_ES: [
        ...
        "¿Sientes que la prisa de los demás dicta el ritmo de tu propia pausa?",
    ...
    cambiarIdioma(lang) {
        ...
        const elBrandSub = document.getElementById('lblBrandSub');
        if (elBrandSub) elBrandSub.innerText = diccionario.brandSub;

        const elTimerTitle = document.getElementById('lblTimerTitle');
        if (elTimerTitle) elTimerTitle.innerText = diccionario.timerTitle;

        const elOraculoInstruccion = document.getElementById('lbl-oraculo-instruccion');
        if (elOraculoInstruccion) elOraculoInstruccion.innerText = diccionario.oraculoInstruccion;

        const elDesahogo = document.getElementById('lbl-desahogo');
        if (elDesahogo) elDesahogo.innerText = diccionario.desahogoLabel;

        const elInpTextLibre = document.getElementById('inp-text-libre');
        if (elInpTextLibre) elInpTextLibre.placeholder = diccionario.placeholderLibre;

        const elBtnActivarLibre = document.getElementById('btn-activar-libre');
        if (elBtnActivarLibre) elBtnActivarLibre.innerText = diccionario.btnActivar;
    ...
    despertarInicial() {
        ...
        const wForm = document.getElementById('wrapper-form');
        if (wForm) wForm.classList.remove('hidden');
    ...
    inyectarPreguntasOraculo() {
        ...
        seleccionadas.forEach((pregunta, idx) => {
            ...
            btn.onclick = () => {
                const campo = document.getElementById('inp-text-libre');
                if (campo) campo.value = pregunta;
                this.ejecutar();
            };
    ...
    async ejecutar() {
        ...
        const zipCode = inpZip ? inpZip.value || "33167" : "33167";

        const modoSelector = document.getElementById('modo-selector');
        const modoActivo = modoSelector ? modoSelector.value : "estandar";

        const menteSelector = document.getElementById('mente-selector');
        const menteActiva = menteSelector ? menteSelector.value : "clima";
        ...
        const perfilActivo = perfilSelector ? perfilSelector.value : "ejecutivo";

        const inpTextLibre = document.getElementById('inp-text-libre');
        const textoLibreStr = inpTextLibre ? inpTextLibre.value : "";
    ...
    actualizarRelojInterfaz() {
        ...
        if (clockDisplay) clockDisplay.innerText = `${mins}:${secs}`;
    },
    ...
    inyectarDestinoOpcionalRojo() {
        console.log("Destino opcional rojo inyectado.");
    },

    activarCierreConscienteCronometrado() {
        this.cierreTimeLeft = 60;
        const dic = this.TRADUCCIONES[this.idiomaActual];
